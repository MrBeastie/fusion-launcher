...
RetroHydra can enrich installed games with metadata and artwork in two layers:
...
2. Built-in compile-time key from `RETROHYDRA_STEAMGRIDDB_KEY`.
...
RetroHydra does not proxy SteamGridDB artwork. Requests go from the user's local client to SteamGridDB, then artwork URLs are cached locally on the user's machine. This preserves the zero-responsibility model for both user-provided and built-in API keys.
