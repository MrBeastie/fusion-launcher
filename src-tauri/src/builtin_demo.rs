...
pub const BUILTIN_DEMO_REPOSITORY_URL: &str = "retrohydra://builtin/demo-repository.json";

const BUILTIN_DEMO_REPOSITORY_JSON: &str = include_str!("../../public/demo-repository.json");
const BUILTIN_DEMO_ROM_PATH: &str = "demo-content/retrohydra-smoke.nes";
const BUILTIN_DEMO_ROM_BYTES: &[u8] =
    include_bytes!("../../public/demo-content/retrohydra-smoke.nes");

pub fn is_builtin_repository_url(url: &str) -> bool {
    url.trim() == BUILTIN_DEMO_REPOSITORY_URL
}
...
mod tests {
    ...
    fn loads_and_verifies_built_in_demo_repository() {
        ...
        assert_eq!(repo.metadata.id, "retrohydra-demo");
        assert_eq!(repo.catalog.len(), 1);
        assert_eq!(repo.catalog[0].id, "retrohydra_nes_smoke");
